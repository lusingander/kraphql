package com.github.lusingander.kraphql.gen.Input
...
class Query(__name: String = "query"): ObjectNode(__name) {
    fun foo(input: FooInput, init: Foo.() -> Unit) =
        Foo("foo").apply { addArgs("input", input) }.also { doInit(it, init) }
    fun fooOpt(input: FooInput? = null, init: Foo.() -> Unit) =
        Foo("fooOpt").apply { addArgs("input", input) }.also { doInit(it, init) }
}
...
class FooInput(val name: String, val value: Int? = null) {
