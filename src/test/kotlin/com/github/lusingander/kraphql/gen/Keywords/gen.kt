package com.github.lusingander.kraphql.gen.Keywords
...
class Foo(__name: String = "Foo"): ObjectNode(__name) {
    val `package` get() =
        ScalarNode("package").also { doInit(it) }
    fun `object`(`class`: ID? = null, `null`: Boolean? = null) =
        ScalarWithArgsNode("object", mapOf("class" to `class`, "null" to `null`)).also { doInit(it) }
    val `object` get() =
        ScalarNode("object").also { doInit(it) }
}
